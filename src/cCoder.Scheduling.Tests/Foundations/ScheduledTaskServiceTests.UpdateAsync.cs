...
public partial class ScheduledTaskServiceTests
{
    ...
    public async Task ShouldDelegateToBrokerWhenUserIsAuthorizedForUpdateAsync()
    {
        ...
        authorizationBrokerMock.Setup(x => x.Authorize((int?)7, "ScheduledTask_update"));

        scheduledTaskBrokerMock
            .Setup(x => x.UpdateScheduledTaskAsync(It.IsAny<ScheduledTask>()))
            .Callback<ScheduledTask>(candidate => submitted = candidate)
            .ReturnsAsync((ScheduledTask value) => value);

        // When
        ScheduledTask result = await scheduledTaskService.UpdateAsync(scheduledTask);
        ...
        scheduledTaskBrokerMock.Verify(
            x => x.UpdateScheduledTaskAsync(It.IsAny<ScheduledTask>()),
            Times.Once
        );
        scheduledTaskBrokerMock.Verify(
        ...
        authorizationBrokerMock.Verify(
            x => x.Authorize((int?)7, "ScheduledTask_update"),
            Times.Once
        );
    }
    ...
    public async Task ShouldThrowSecurityExceptionWhenUserLacksUpdatePrivilegeForUpdateAsync()
    {
        ...
        authorizationBrokerMock
            .Setup(x => x.Authorize((int?)7, "ScheduledTask_update"))
            .Throws(new SecurityException("Access Denied!"));

        // When
        Func<Task> action = async () => await scheduledTaskService.UpdateAsync(scheduledTask);
