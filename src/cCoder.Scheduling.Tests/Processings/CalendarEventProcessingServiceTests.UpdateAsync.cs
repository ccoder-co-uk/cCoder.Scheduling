...
public partial class CalendarEventProcessingServiceTests
{
    ...
    public async Task ShouldDelegateToFoundationServiceWhenUpdateAsync()
    {
        ...
        calendarEventServiceMock.Setup(x => x.UpdateAsync(entity)).ReturnsAsync(entity);

        // When
        CalendarEvent result = await calendarEventProcessingService.UpdateAsync(entity);

        // Then
        result.Should().BeSameAs(entity);
        calendarEventServiceMock.Verify(x => x.UpdateAsync(entity), Times.Once);
