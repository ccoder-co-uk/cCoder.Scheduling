...
internal class SchedulingMigrationAggregationService(
    ...
    ILogger<SchedulingMigrationAggregationService> logger,
    IJsonBroker jsonBroker
) : ISchedulingMigrationAggregationService
{
    public async ValueTask ImportPackageAsync(int appId, SchedulingPackage package)
    {
        if (package.Items is null || package.Items.Count == 0)
            return;

        foreach (SchedulingPackageItem item in package.Items)
        {
            switch (item.Type)
            {
                case "Core/Calendar":
    ...
    public SchedulingPackage ExportPackage(int appId, string packageName)
    {
        ...
        return new SchedulingPackage(package.Name)
        {
            Id = package.Id,
            Name = package.Name,
            ...
            Items = package.Items?
                .Select(item => new SchedulingPackageItem
                {
                    Id = item.Id,
    ...
    private async ValueTask ImportCalendarsAsync(int appId, SchedulingPackageItem item)
    {
    ...
    private async ValueTask ImportCalendarEventsAsync(int appId, SchedulingPackageItem item)
    {
